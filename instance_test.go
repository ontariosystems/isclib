...
var _ = Describe("Instance", func() {
	Context("InstanceFromQList", func() {
		Context("Invalid qlist", func() {
			instance, err := isclib.InstanceFromQList("1^2^3^4^5^6^7")
		...
		Context("Valid old qlist", func() {
			instance, err := isclib.InstanceFromQList("INSTTEST^/ensemble/instances/insttest/^2012.2.3.903.2.12515^down, last used Thu Sep 15 18:58:30 2016^cache.cpf^56772^57772^62972")
			...
			It("Populates the instance properly from a qlist", func() {
				...
				Expect(instance.Status).To(Equal(isclib.InstanceStatusDown), "status")
		...
		Context("Valid qlist", func() {
			instance, err := isclib.InstanceFromQList("INSTTEST^/ensemble/instances/insttest/^2015.2.2.805.0.16216^running, since Fri May 13 22:07:02 2016^cache.cpf^56772^57772^62972^warn^")
			It("Does not return an error", func() {
				Expect(err).NotTo(HaveOccurred())
			})
			It("Populates the instance properly from a qlist", func() {
		...
		Context("Valid Durable %SYS qlist", func() {
			instance, err := isclib.InstanceFromQList("INSTTEST^/ensemble/instances/insttest/^2015.2.2.805.0.16216^running, since Fri May 13 22:07:02 2016^cache.cpf^56772^57772^62972^ok^^^^/mgr/config")
			It("Does not return an error", func() {
				Expect(err).NotTo(HaveOccurred())
			})
			It("Populates the instance properly from a qlist", func() {
				...
				Expect(instance.JDBCPort).To(Equal(62972), "jdbc port")
				Expect(instance.State).To(Equal("ok"), "state")
				Expect(instance.Product).To(Equal(isclib.Cache), "product")
				Expect(instance.DataDirectory).To(Equal("/mgr/config"), "data directory")
			})
		})
		Context("Valid Mirroring", func() {
			instance, err := isclib.InstanceFromQList("INSTTEST^/ensemble/instances/insttest/^2015.2.2.805.0.16216^running, since Fri May 13 22:07:02 2016^cache.cpf^56772^57772^62972^ok^^Failover^Primary^/mgr/config")
			It("Does not return an error", func() {
				Expect(err).NotTo(HaveOccurred())
			})
			It("Populates the instance properly from a qlist", func() {
